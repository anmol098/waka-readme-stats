...
# Métriques de développement dans le README avec drapeaux de fonctionnalités 🎌
...
<p align="center">
   ...
   <img src="https://img.shields.io/static/v1?label=%F0%9F%8C%9F&message=Si+utile&style=flat&color=BC4E99" alt="Badge d'étoiles">
</p>

<p align="center">
   Êtes-vous un oiseau de jour 🦉 ou une nuit 🐤 ?
   <br/>
   À quelle heure de la journée êtes-vous le plus productif ?
   <br/>
   Quels sont les langages que vous utilisez pour coder ?
   <br/>
...
1. Vous devez mettre à jour le fichier Markdown (`.md`) avec 2 commentaires. Consultez [ici](#mise-à-jour-de-votre-readme) pour des instructions sur la mise à jour.
2. Vous aurez besoin d'une clé API WakaTime. Vous pouvez l'obtenir depuis vos paramètres de compte WakaTime.
   - Consultez [ici](#nouveau-pour-wakatime) si vous êtes nouveau chez WakaTime.
3. Vous aurez besoin d'un jeton d'accès GitHub avec les étendues `repo` et `user` depuis [ce lien](https://github.com/settings/tokens).
   - Utilisez [cette](#profil-du-dépôt) exemple pour vous aider.
...
1. Enregistrez la clé API WakaTime et le jeton d'accès GitHub (PAT) en tant que secrets du dépôt. Vous pouvez trouver cette option dans les paramètres de votre dépôt.
   - Enregistrez-les comme suit :
     - Clé API WakaTime : `WAKATIME_API_KEY=<votre clé API WakaTime>`
     - Jeton d'accès PAT GitHub : `GH_TOKEN=<votre jeton d'accès GitHub>`
2. Activez et désactivez les drapeaux de fonctionnalité en fonction de vos besoins.

Cette action GitHub peut être configurée pour s'exécuter à tout moment souhaité grâce à `cron`. Consultez [Crontab.guru](https://crontab.guru/) et [ce site](https://crontab.cronhub.io/) pour générer des expressions cron.
...
## Nouveau(elle) chez WakaTime

WakaTime vous donne une idée du temps réel que vous passez à coder. Cela vous aide à améliorer votre productivité et à rester en avance sur la concurrence.
...
Si vous souhaitez ajouter d'autres informations à vos statistiques, vous pouvez ajouter plusieurs `FLAGS` dans votre fichier de workflow. Par défaut, tous les flags sont activés (à l'exception du flag lignes de code en raison de l'opération lourde effectuée).
...
Le `SECTION_NAME` peut être défini sur n'importe quelle chaîne, et sera le nom de la section à remplacer dans le README.

Le `COMMIT_BY_ME` peut être défini sur `True` pour commettre le code avec votre nom et adresse e-mail.

Le `COMMIT_MESSAGE` peut être personnalisé pour le message de commit. La valeur par défaut est "Mise à jour avec les métriques de développement".

Le `COMMIT_USERNAME` peut être défini comme un nom d'utilisateur pour commettre le code. La valeur par défaut est "readme-bot".

Le `COMMIT_EMAIL` peut être défini pour une adresse e-mail afin de commettre le code. La valeur par défaut est "41898282+github-actions[bot]@users.noreply.github.com".

Le `SHOW_UPDATED_DATE` peut être défini sur `True` pour afficher la date de mise à jour en fin de paragraphe.

Le `UPDATED_DATE_FORMAT` peut être personnalisé pour formater la date de mise à jour. La valeur par défaut est `"%d/%m/%Y %H:%M:%S"`.

Le `SHOW_LINES_OF_CODE` peut être défini sur `True` pour afficher le nombre de lignes de code écrites jusqu'à présent.

![Lignes de Code](https://img.shields.io/badge/De Hello World à ce jour-1,3 million de lignes de code-blue)

Le `SHOW_TOTAL_CODE_TIME` peut être défini sur `False` pour masquer *Temps de codage*.

![Temps de Codage](http://img.shields.io/badge/Temps%20de%20codage-1%2C438 heures 54 minutes-blue)

Le `SHOW_PROFILE_VIEWS` peut être défini sur `False` pour masquer **Vues de profil**.

![Vues de Profil](http://img.shields.io/badge/Vues%20de%20profil-2189-blue)

Le `SHOW_COMMIT` peut être défini sur `False` pour masquer les statistiques de commit.
...
Le drapeau `SHOW_TIMEZONE` peut être défini sur `False` pour masquer la zone horaire dans laquelle vous êtes.

```text
⌚︎ Timezone: Asia/Calcutta
```

Le drapeau `SHOW_EDITORS` peut être défini sur `False` pour masquer la liste des éditeurs de code/IDE utilisés.
...
## :coeur_en_feux: Soutenez le projet
...
Cependant, si vous utilisez ce projet et que vous êtes satisfait ou simplement que vous voulez m'encourager à continuer de créer des choses, il y a quelques façons de le faire :

- Donner le crédit approprié lorsque vous utilisez cette action dans votre README, en liant vers celui-ci :)
- Ajouter une étoile et partager le projet 🚀
- [![paypal.me/aapreneur](https://ionicabizau.github.io/badges/paypal.svg)](https://www.paypal.me/aapreneur) - Vous pouvez faire des dons ponctuels via PayPal. Je vais probablement acheter de la ~~bière~~ du vin 🍷.

Merci ! :coeur:

# Contributions

Les contributions sont les bienvenues ♥ ! Veuillez partager toutes fonctionnalités et ajouter des tests unitaires ! Utilisez les systèmes de requêtes de tirage et d'issues pour contribuer.
...
10. [Ss5h](https://github.com/tlatkdgus1) : Pour l'ajout du support de la rédaction de phrases naturelles pour la traduction [#136](https://github.com/anmol098/waka-readme-stats/pull/136)
...
- Et vous ! Si vous l'utilisez actuellement et que vous n'êtes pas sur la liste, veuillez nous le faire savoir en créant un [mention spéciale](https://github.com/anmol098/waka-readme-stats/issues/new/choose) ! :blush: Nous serons ravis de vous ajouter à la liste.
