...
# README に Dev Metrics を表示し、追加の機能フラグを有効にしました 🎌
...
2. WakaTime APIキーが必要です。これはWakaTimeアカウント設定から取得できます。
    - WakaTimeに新規で登録されている場合は[こちら](#new-to-wakatime)を参照してください。
...
1. リポジトリのシークレットに WakaTime API Key と GitHub API Token を保存する必要があります。これは、リポジトリの Settings で確認できます。  
  以下の通りに保存してください：
    - WakaTime API Key を `WAKATIME_API_KEY=<your wakatime API Key>` として
    - GitHub Personal Access Token (PAT) を `GH_TOKEN=<your github access token>` として
2. ご要望に応じて、機能フラグを有効または無効にできます。

この GitHub Action は、`cron` を使用していつでも実行できます。`cron` 表記を生成するには [Crontab.guru](https://crontab.guru/) および [this](https://crontab.cronhub.io/) のサイトをご覧ください。

## Readmeの更新

`README.md`にコメントを追加するようにしてください。以下のようにします：
...
WakaTimeは、実際にコーディングに費やした時間を把握するのに役立ちます。これにより、生産性と競争力を高めることができます。
...
GitHub Access Token を取得する必要があります。`repo` および `user` スコープを持つトークンを取得し、Repo Secrets に `GH_TOKEN = <Your GitHub Access Token>` として保存してください。
...
- 今からコミットして、自動で実行を待つこともできますし、すぐに結果を確認するために手動で実行することもできます。リポジトリ内の `Actions` にアクセスし、`Profile Readme Development Stats` のワークフローを選択し、`Run workflow` をクリックしてください。1〜2分待つと、変更内容が反映されます。

## Extras

他の情報をあなたの統計に追加したい場合は、ワークフロー ファイルに複数の `FLAGS` を追加できます。デフォルトではすべてのフラグが有効になっています（コード行数フラグは、実行される処理が重いため、例外です）。
...
`LOCALE` このフラグは、統計をあなたの言語で表示するために使用できます。デフォルトは英語です。フラグ変数にLocale [略称](https://saimana.com/list-of-country-locale-code/)を渡してください。最終的な結果の例は[こちら](https://github.com/anmol098/anmol098/blob/master/Readme-fr.md)にあります。

`SECTION_NAME` フラグは任意の文字列に設定でき、READMEに置き換えるセクションの名前になります。

`COMMIT_BY_ME` フラグを`True`に設定すると、あなたの名前とメールアドレスを使用してコードをコミットします。
...
`UPDATED_DATE_FORMAT` フラグを設定すると、更新日時を指定された形式に変換できます。デフォルトは`"%d/%m/%Y %H:%M:%S"`です。
...
`SHOW_LANGUAGE`フラグを`False`に設定すると、使用するプログラミング言語を非表示にできます。
...
`SHOW_PROJECTS`フラグを`False`に設定することで、作業中のプロジェクトを非表示にできます。
...
`SHOW_EDITORS` フラグを `False` に設定して、使用されているコードエディタ/IDEの一覧を非表示にできます。
...
`SHOW_LANGUAGE_PER_REPO`フラグを`False`に設定すると、さまざまなプログラミング言語やフレームワークごとのリポジトリ数を非表示にできます。
...
`SHOW_SHORT_INFO` フラグを `False` に設定することで、ユーザーの短い面白い事実情報を非表示にできます。
...
`SHOW_LOC_CHART`フラグを`False`に設定することで、年ごとの四半期ごとのコード行数のグラフを非表示にできます。

`IGNORED_REPOS`フラグを`"waka-readme-stats, my-first-repo"`（例）に設定することで、カウントから除外したいリポジトリを指定できます。

`SYMBOL_VERSION`フラグを設定することで、進行状況バーのシンボルを変更できます（デフォルト: `1`）。
...
![チャートが見つからない](https://raw.githubusercontent.com/anmol098/anmol098/master/charts/bar_graph.png)

## :sparkling_heart: プロジェクトをサポートする

私はできる限りすべてをオープンソース化しており、これらのプロジェクトを使って助けが必要な人々にできるだけ応答しようと努めています。明らかに、これは時間がかかります。このサービスは無料で利用できます。

しかし、このプロジェクトを使用し、満足している場合、または私の創作活動を継続するのを応援したい場合、いくつかの方法があります :-

- このアクションを使用する際、READMEに適切なクレジットを記載し、リンクを戻すことで :D
- プロジェクトをスター付きにし、共有する :rocket:
- [![paypal.me/aapreneur](https://ionicabizau.github.io/badges/paypal.svg)](https://www.paypal.me/aapreneur) - PayPalを通じて一時的な寄付が可能です。私はおそらく~~ビール~~ワイン 🍷を購入するでしょう。

ありがとう！:heart:

# 貢献

貢献は歓迎されます♥! ご希望の機能や、ユニットテストをぜひご提供ください！プルリクエストやイシューのシステムを使ってご貢献ください。
...
10. [Ss5h](https://github.com/tlatkdgus1): 翻訳用自然な文の書き方のサポートの追加 [#136](https://github.com/anmol098/waka-readme-stats/pull/136)
...
- そしてあなたも！もし今現在使用しているがリストに載っていない場合は、[Special Mention](https://github.com/anmol098/waka-readme-stats/issues/new/choose) のissueを送ってください！:blush: \
  リストに加えていただけることに喜びを感じます。
