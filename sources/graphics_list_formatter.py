...
async def make_commit_day_time_list(time_zone: str, repositories: Dict, commit_dates: Dict) -> str:
    ...
    for repository in [d for d in repositories["data"]["user"]["repositories"]["nodes"] if d]:
...
def make_language_per_repo_list(repositories: Dict) -> str:
    ...
    """
    language_count = dict()
    repos_with_language = [repo for repo in repositories["data"]["user"]["repositories"]["nodes"] if repo and repo["primaryLanguage"] is not None]
